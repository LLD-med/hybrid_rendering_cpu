...
void ModelCommand::setAdapter()
{
    auto adapter = std::make_shared<ModelTransformAdapter>();
...
//    ((*_transformManager).*_method)(_object, _dx, _dy, _dz);
    _object->setMoveCoefs(_dx, _dy, _dz);
}
...
//    ((*_transformManager).*_method)(_object, _kx, _ky, _kz);
    _object->setScaleCoefs(_kx, _ky, _kz);
}
...
//    ((*_transformManager).*_method)(_object, _ox, _oy, _oz);
    _object->setRotateCoefs(_ox, _oy, _oz);
}


TransformModel::TransformModel(const std::shared_ptr<BaseObject> object, Transform &mtr) :
...
{
    _method = &SceneManager::deleteCamera;
}
